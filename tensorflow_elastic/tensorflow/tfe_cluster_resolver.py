...
def _load_tf_config():
  cspec = json.loads(os.environ.get(_TF_CONFIG_ENV, '{}'))
  if("cluster" in cspec and "worker" in cspec["cluster"]):
    if(isinstance(cspec["cluster"]["worker"],dict)):
      new_dict = {int(key):cspec["cluster"]["worker"][key] for key in cspec["cluster"]["worker"]}
      cspec["cluster"]["worker"] = new_dict
  return cspec
