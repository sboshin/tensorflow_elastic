import os
#os.environ["TF_CPP_VMODULE"] ="collective_param_resolver_distributed=4,collective_param_resolver_local=4,collective_ops=4,base_collective_executor=4,hierarchical_tree_broadcaster=4"
#os.environ["TF_CPP_VMODULE"] ="constant_folding=1"
#os.environ["TF_CPP_VMODULE"] ="collective_param_resolver_local=4"
...
from tensorflow_elastic.api import api
from tensorflow_elastic import utils
import socket
...
def parse_args():
    ...
    parser.add_argument(
      "--epochs", type=int, help="Number of epochs", default=5
    )
    ...
    parser.add_argument(
      "--min_nodes", type=int, help="min number of nodes", default=2
    )

    parser.add_argument(
      "--max_nodes", type=int, help="min number of nodes", default=3
    )
...
@tf.function
def train_step(strategy, multi_worker_model, global_batch_size, optimizer, train_loss, train_accuracy, iterator=None):
  """See base class."""
...
def main(**kwargs):
    ...
    if("proc_id" in kwargs):
      ...
      local_address = socket.gethostname()+":5555"
    
    api.init(local_address, args.min_nodes, args.max_nodes)
    ...
    print(f"Global batch size is {global_batch_size} and steps {args.steps} num workers {num_workers}")
    _, test_dataset = cifar_dataset(global_batch_size, args.steps)
    ...
    tmp_dir = "/tmp/backup"
    s3_path = "s3://sam-debug-binaries/elastic_norestart/new_design"
    ...
    manager = tf.train.CheckpointManager(checkpoint, directory=backup_fdr, max_to_keep=1)
    ...
    def get_dataset():
      ...
      num_workers = strategy.extended._cluster_resolver.cluster_spec().num_tasks("worker")
      global_batch_size = per_worker_batch_size * num_workers
      return cifar_dataset(global_batch_size, args.steps)[0]

    #Someone should get unique index 0
    tf_config = json.loads(os.environ["TF_CONFIG"])
    print(tf_config["task"]["index"])
    if(int(tf_config["task"]["index"]) == 0):
      wbackup_fdr = backup_fdr
    else:
      wbackup_fdr = backup_fdr+local_address.strip().replace(":","")
    
    print(f"{tf_config['task']['index']} saving to {wbackup_fdr}")
    checkpoint.write(wbackup_fdr)
    
    print(f"Setup took {time.time() - main_start} s")
    while epoch < args.epochs:
      
      epoch_start = time.time()
      run_args = (strategy, multi_worker_model, global_batch_size, optimizer, train_loss, train_accuracy)
      
      api.run(train_step, run_args, get_dataset,strategy, manager)    
      print(f"Finished Epoch {epoch.numpy()} in {time.time() - epoch_start}\n\n", flush=True)
      print(f"Train Loss result is {train_loss.result().numpy()}")
      print(f"Train Accuracy is {train_accuracy.result().numpy()}")
      
      try:
        epoch.assign_add(1)
        save(strategy, backup_fdr, checkpoint, local_address)
    ...
    print(f"Finished Training successfully took {(time.time() - main_start)/60.0} mins, returning", flush=True)
