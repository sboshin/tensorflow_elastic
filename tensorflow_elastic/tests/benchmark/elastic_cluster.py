...
def run_script(hostname, key, tfconfig, script, args, local, log_dir,private_ip, add_python=True):
  with utils.setup_env(hostname, key) as connection:
    ...
    if(add_python):
      connection.run(f"python {remote_script} {args} |& tee -a {worker_log}", env=myenv)
...
class ElasticCluster():
  def __init__(self, instance_type, instance_cnt, ami, key, setup_script, elastic_pattern, training_script_params, local, log_dir="./"):
  ...
  def terminate_scripts_on_hosts(self):
    ...
    for hostname in self._hostnames:
      mp_args.append((hostname, self._key, {}, "killall python", "", True, self._log_dir, hostname, False))
    try:
  ...
  def run_pattern(self):
    ...
    if(pattern == "cycle"):
      ...
      while(True):
        pool = Pool(processes=len(self._hostnames))
        #Setup Run
        hostname_clusterspec = utils.setup_tf_config(self._hostnames, self._private_ips, 5555)
        mp_args = []
        for hostname in hostname_clusterspec:
          ...
          mp_args.append((hostname, self._key, hostname_clusterspec[hostname], 
            self._training_script_params["script"], self._training_script_params["args"], self._local, self._log_dir, private_ip))
